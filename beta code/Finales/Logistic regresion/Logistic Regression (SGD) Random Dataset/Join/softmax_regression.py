...
for k in range(20):
	...
	t1=time.clock()
	timeGraNum.append(t1-t0)
	t2=time.clock()
	auto_grad=training_gradient_fun(params)
	t3=time.clock()
	timeAutograd.append(t3-t2)
	addL2Norm.append(LA.norm(auto_grad[0].flatten()-n_grad[0].flatten()))
...
'X_TimeGraNum':timeGraNum,'Y_TimeAutograd':timeAutograd,'Z_AddL2Norm':addL2Norm}
